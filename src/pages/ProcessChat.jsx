...
export default function ProcessChat() {
  ...
  useEffect(() => {
    ...
    // If loading an already-processed entry, mark as finished so auto-save doesn't fire
  ...
  useEffect(() => {
    ...
    // Don't save if the entry is already complete OR if we just finished it
  ...
  }, [messages, apiHistory, currentStep, stepData, activeChips, hasInitialized, isFinished]);
  ...
  const finishProcessing = (finalEntry) => {
    ...
    // Mark as finished FIRST so the auto-save effect doesn't overwrite our save
    setIsFinished(true);
    ...
    // Update local entry state so subsequent renders see the new status
