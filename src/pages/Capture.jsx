...
export default function Capture() {
  ...
  // For now use default chips; later this will be AI-suggested based on situation text
  ...
  const canSave = situation.trim().length > 0 && selectedWounds.length > 0 && selectedEmotions.length > 0;
  ...
  const processNow = () => {
    if (!canSave) return;
    // For now, save it and notify — chat flow comes in next iteration
    saveEntry({
      status: 'captured',
      situation: situation.trim(),
      wounds: selectedWounds,
    ...
    alert('Saved. Process flow (chat) is coming in the next iteration.');
    navigate('/');
  };
    ...
    <div className="fade-in">
      ...
      <div className="app-content" style={{ paddingTop: 'var(--space-5)' }}>
        ...
        <div className="section">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-2)' }}>
            <label className="field-label" style={{ marginBottom: 0 }}>Wound · pick one or more</label>
            <button onClick={() => setShowAllWounds(true)} style={{ fontSize: '11px', color: 'var(--text-secondary)', textDecoration: 'underline' }}>
              Show all
            </button>
          </div>
          <ChipRow
        ...
        <div className="section">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-2)' }}>
            ...
            <button onClick={() => setShowAllEmotions(true)} style={{ fontSize: '11px', color: 'var(--text-secondary)', textDecoration: 'underline' }}>
              Show all
            </button>
          </div>
          <ChipRow
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <button className="btn-secondary" onClick={saveForLater} disabled={!canSave} style={{ opacity: canSave ? 1 : 0.4 }}>
            Save for later
          </button>
        </div>
        <p className="field-helper" style={{ textAlign: 'center', marginTop: 'var(--space-3)' }}>
          Process now picks up where this leaves off.<br />
          Save for later puts it on your home screen.
        </p>
      </div>
