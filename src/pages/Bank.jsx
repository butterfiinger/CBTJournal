      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          <p className="empty-state-message">
            {filter === 'good_moments' && 'No good moments banked yet.'}
            {filter === 'processed' && 'No processed entries yet.'}
            {filter === 'all' && 'Nothing banked yet.'}
          </p>
          <p className="empty-state-helper">
            When you log a good moment or process a trigger, it lands here.
          </p>
        </div>
      ) : (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {filtered.map((entry) => (
              <div key={entry.id} className="card" style={{ padding: 'var(--space-4)' }}>
                ...
                <p style={{ fontSize: '14px', marginBottom: 'var(--space-2)', lineHeight: 1.5 }}>
                  {entry.situation}
                </p>
